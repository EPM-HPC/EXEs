...
#================PTX & SASS
PTX="/home/wwr/Desktop/EXEs/PTXs/ExeGM-akhi-stall"
SASS="/home/wwr/Desktop/EXEs/SASSs/ExeGM-akhi-stall"
...
#================PROFILE related
PROF="/home/wwr/Desktop/EXEs/PROFs/ExeGM-akhi-stall/"
...
for sm_freq in  ${SM[@]};do
    ...
    for file in *.out;do
        $ncu_pre $PROF"ncuprof_output_"$file"_freq_"$sm_freq $ncu_param ./$file $NumCTA $NumThd $NumIter 16 1 >/dev/null 2>&1	
        echo "[*] profiling " $file " at "$sm_freq "in level 1"
    done
    sudo nvidia-smi -rgc
done
echo "<<<<<<<<<<profile stage done>>>>>>>>>>"
...
ENERGY="/home/wwr/Desktop/EXEs/ENERGYs/ExeGM-akhi-stall/"
for sm_freq in  ${SM[@]};do
        sudo nvidia-smi -lgc $sm_freq
        for file in *.out;do
            ...
            $PM 1 > $efilename &
            PM_PID=$!
            ./$file $NumCTA $NumThd $NumIter 16 1 
            sleep 1
